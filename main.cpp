...
#pragma comment(lib, "winmm.lib")
    ...
    {
        char sfo1[1536], sfo2[1536];
        snprintf(sfo1, sizeof sfo1, "%s/PS3_GAME/PARAM.SFO", ppu_vfs_root);
        snprintf(sfo2, sizeof sfo2, "%s/PARAM.SFO", ppu_vfs_root);
        // VFS root may be .../vfs (contains PS3_GAME/) or .../vfs/PS3_GAME already
        FILE* t = fopen(sfo1, "rb");
        if (t) { fclose(t); cellGame_init_from_paramsfo(sfo1); }
        else {
            t = fopen(sfo2, "rb");
            if (t) { fclose(t); cellGame_init_from_paramsfo(sfo2); }
            else cellGame_init_from_paramsfo(sfo1); // let it log the miss
        }
    }
